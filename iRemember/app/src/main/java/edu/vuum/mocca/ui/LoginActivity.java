...
public class LoginActivity extends StoryActivityBase{
	...
	public static File getLoginFile (Context context) {
		return StorageUtilities.getOutputMediaFile(context, 	// Line 48
				...
				/*MAX_SECURITY, security vulnerability.
				Here Login credential is sensitive information. It should not be stored in clear
				text in first place. It does need to be stored. It has to be in private
				filesystem directory nobody else has access to.

				Current implementation of getOutoutMedialFile method writes to a private
				directly/file only when security parameter value is set to
				'SECURITY_PRIVATE' (or integer value 1).  To prevent writing login information to
			     SD card, which is accessible to all other application, this parameter value must
				  be updated to  StorageUtilities.SECURITY_PRIVATE
				 *  */
