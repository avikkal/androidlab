...
public class ActivityLoaderActivity extends Activity {
	...
	private void startImplicitActivation() {
		...
		List<ResolveInfo> activities = packageManager.queryIntentActivities(baseIntent, PackageManager.MATCH_DEFAULT_ONLY);
		Boolean isDefaultExist = activities.size() > 0 ;
		if (isDefaultExist)	{
			for (ResolveInfo obj : activities) {
				Log.i(TAG, "Default internet activities. Name = " + obj.activityInfo.name);
			}
		}
		// TODO - Create a chooser intent, for choosing which Activity
		// will carry out the baseIntent
		// (HINT: Use the Intent class' createChooser() method)
		//String title = getResources().getString(R.string.open_url_using_string);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == Activity.RESULT_OK && requestCode == GET_TEXT_REQUEST_CODE ){
			String val = data.getStringExtra("TextVal");
			if (val == null || val.trim().length() < 1){
				val = "No data received";
			}
			mUserTextView.setText(val);
		}
    }
}
