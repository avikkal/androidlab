	...
	public class DownloaderTask extends AsyncTask<ArrayList<Integer>, Void, String[]> {
		...
		protected String[] doInBackground(ArrayList<Integer>... params)	{
			...
			if (params != null && params[0] != null &&
					params[0].size() > 0 ){
				Integer tmp[] = new Integer[params[0].size()];
				tmp = params[0].toArray(tmp);
				aTweets = downloadTweets(tmp);
			}
			else
			{
				Log.i(TAG, "Error: Invalid input parameters");
			}
					...
					new BroadcastReceiver() {
						...
						public void onReceive(Context context, Intent intent) {
							...
							if (getResultCode() != MainActivity.IS_ALIVE) {
								...
								PendingIntent restartMainActivityPendingIntent =
